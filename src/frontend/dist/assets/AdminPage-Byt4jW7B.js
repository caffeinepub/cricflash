...
async function sendToTelegram(article) {
  ...
  if (!res.ok) {
    ...
    throw new Error((data == null ? void 0 : data.description) || `Telegram API error: ${res.status}`);
  }
}
...
function AdminPage() {
  ...
  const anyAutomationRunning = isFetchingMatches || isGenerating || isPublishing;
